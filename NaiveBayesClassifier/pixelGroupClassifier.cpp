...
pixelGroupClassifier::pixelGroupClassifier(float fSmoothingConstant, int fFeatureSet)
{
	...
	*/
	...
	{
		numOfPixelGroupRow = 28 / m;
		numOfPixelGroupCol = 28 / n;
	}
	else //(featureSetOverlapping == true)
	{
		numOfPixelGroupRow = 29 - m;
		numOfPixelGroupCol = 28 - n;
	}
	...
	for (int i = 0; i < 10; i++)
	{
		pixelGroupProbability[i] = new float** [numOfPixelGroupRow];

		for (int j = 0; j < (numOfPixelGroupRow); j++)
		{
			pixelGroupProbability[i][j] = new float* [numOfPixelGroupCol];

			for (int k = 0; k < (numOfPixelGroupCol); k++)
			{
...
void pixelGroupClassifier::trainModel()
{
	...
	for (int i = 0; i < 10; i++)
	{
		pixelGroupCountMatrices[i] = new float** [numOfPixelGroupRow];

		for (int j = 0; j < (numOfPixelGroupRow); j++)
		{
			pixelGroupCountMatrices[i][j] = new float* [numOfPixelGroupCol];
	...
	*/
	...
	for (int i = 0; i < 5000; i++)
	{
		...
		*/
		...
		for (int j = 0; j < 10; j++)
		{
			for (int k = 0; k < (numOfPixelGroupRow); k++)
			{
				for (int l = 0; l < (numOfPixelGroupCol); l++)
				{
		...
		for (int j = 0; j < 10; j++)
		{
			...
			for (int k = 0; k < (numOfPixelGroupRow); k++)
			{
				for (int l = 0; l < (numOfPixelGroupCol); l++)
				{
					...
					int tempPixelGroupNumber;

					if (featureSetDisjoint == true)
					{
						tempPixelGroupNumber = getPixelGroupNumber(nextTestImage, (k * m), (l * n));
					}
					else //if (featureSetOverlapping == true)
					{
						tempPixelGroupNumber = getPixelGroupNumber(nextTestImage, k, l);
					}
